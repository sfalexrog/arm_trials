...
cd ${TMPDIR}
wget -c --quiet http://raspbian.raspberrypi.org/raspbian/pool/main/u/udev/libudev0_175-7.2_armhf.deb
wget -c --quiet http://raspbian.raspberrypi.org/raspbian/pool/main/s/speech-dispatcher/speech-dispatcher_0.9.1-2_armhf.deb
wget -c --quiet http://raspbian.raspberrypi.org/raspbian/pool/main/libs/libsdl2/libsdl2-2.0-0_2.0.10+dfsg1-1_armhf.deb
...
cd ${TMPDIR}
wget -c --quiet http://raspbian.raspberrypi.org/raspbian/pool/main/d/directfb/libdirectfb-1.7-7_1.7.7-9_armhf.deb
mkdir libdirectfb
dpkg -x libdirectfb-1.7-7_1.7.7-9_armhf.deb libdirectfb
cp -L libdirectfb/usr/lib/arm-linux-gnueabihf/libdirectfb-1.7.so.7 ${APPDIR}/usr/lib/arm-linux-gnueabihf/
cp -L libdirectfb/usr/lib/arm-linux-gnueabihf/libfusion-1.7.so.7 ${APPDIR}/usr/lib/arm-linux-gnueabihf/
cp -L libdirectfb/usr/lib/arm-linux-gnueabihf/libdirect-1.7.so.7 ${APPDIR}/usr/lib/arm-linux-gnueabihf/

# copy libts-0.0-0
wget -c --quiet http://raspbian.raspberrypi.org/raspbian/pool/main/t/tslib/libts0_1.19-1_armhf.deb
mkdir libts
dpkg -x libts0_1.19-1_armhf.deb libts
cp -L libts/usr/lib/arm-linux-gnueabihf/libts.so.0 ${APPDIR}/usr/lib/arm-linux-gnueabihf/
...
tar -C ${tmpdir} --exclude='release/package' -cjf ${TMPDIR}/QGroundControl-arm64.app.tar.bz2 ${APP}".AppDir" --transform 's/release/qgroundcontrol/'
cp ${TMPDIR}/QGroundControl-arm64.app.tar.bz2 ${OUTPUT_DIR}/QGroundControl-arm64.app.tar.bz2
